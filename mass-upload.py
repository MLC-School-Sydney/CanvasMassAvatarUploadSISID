...
for user_image in csv_file_reader:
  ...
  # Step 0: (Duy) Get Canvas User ID via SIS ID
  params = {}
  userDetails = requests.get("https://%s/api/v1/users/sis_user_id:%s"%(domain,'{0}'.format(user_image['user_id'])),headers=header,params=params)
  if 'id' in userDetails.json():
    user_image['user_id'] = userDetails.json()['id']
  else:
    log("### User Not found %s"%(user_image['user_id']))
    continue;
  ...
  if not os.path.exists(image_path):
    print('##### Image Not found - %s'%(user_image['image_filename']))
    continue
  ...
  for ao in avatar_options.json():
    ...
    if ao.get('display_name')==user_image['image_filename']:
      ...
      if set_avatar_user.status_code == 200:
        print('success uploading user avatar for {0}'.format(user_image['user_id']))
